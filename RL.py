...
def learn_Q(env, n_sims, gamma = 1, omega = 0.77, epsilon = 0.05,
    ...
    #######################################################################
            ...
            c = 0.2
            alpha = c / (state_action_count[state][action] ** omega)  # learning rate
            Q[state][action] += alpha * (action_reward + gamma * (not done) * np.max(Q[state2]) - Q[state][action])  # formula from page 81 LN. 
...
def learn_MC(env, n_sims, gamma = 1, epsilon = 0.05,
    ...
    for episode in range(1,n_sims + 1):
     ...
     #################################################################### 
        ...
        for state, actions in episode_state_action_count.items():
            for action, count in enumerate(actions):
                if count > 0:  # visited state
                    G_t = episode_reward
                    Q[state][action] += (G_t - Q[state][action])/state_action_count[state][action]  # update using formula on page 79 in LN. 
